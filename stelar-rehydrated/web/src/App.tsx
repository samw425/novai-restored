...
// ============================================================================
// LANDING PAGE - Entry screen with existing red/black/dark design
// ============================================================================
        ...
        <div className="min-h-screen bg-black flex flex-col items-center justify-center px-6 relative overflow-hidden">
            ...
            <div className="relative z-10 text-center max-w-2xl">
                ...
                <div className="flex items-center justify-center gap-4 mb-8">
                    <div className="w-16 h-16 bg-[#FF4500]/20 rounded-2xl flex items-center justify-center border border-[#FF4500]/30">
                        <Radio className="w-8 h-8 text-[#FF4500]" />
                    </div>
                </div>
                ...
                <p className="text-slate-400 text-lg md:text-xl mb-12 leading-relaxed">
                    Proprietary music intelligence platform identifying the world's next icons before the market.
                </p>

                <button
                    onClick={onEnter}
                    className="px-12 py-5 bg-[#FF4500] text-black font-black text-sm uppercase tracking-[0.2em] rounded-full hover:bg-[#FF4500]/90 hover:scale-105 transition-all shadow-[0_0_40px_rgba(255,69,0,0.4)]"
                >
                    Enter Discovery
                </button>

                <p className="mt-8 text-slate-600 text-xs uppercase tracking-widest">
                    3,660+ Artists • Real-Time Rankings • Proprietary Signals
                </p>
            </div>
        </div>
    );
...
// ============================================================================
// A&R LANDING PAGE - B2B entry for labels
// ============================================================================
const ARLandingPage = ({ onClose, onJoinWaitlist }: { onClose: () => void; onJoinWaitlist: () => void }) => {
    return (
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-6">
            <div className="bg-[#12121a] border border-white/10 rounded-2xl max-w-lg w-full p-8 relative">
                <button onClick={onClose} className="absolute top-4 right-4 text-slate-500 hover:text-white">
                    <X className="w-5 h-5" />
                </button>

                <div className="text-center">
                    <Crown className="w-12 h-12 text-[#FF4500] mx-auto mb-4" />
                    <h2 className="text-3xl font-black text-white uppercase tracking-tight mb-4">For Labels & A&R</h2>
                    <p className="text-slate-400 mb-8">
                        Get early access to our enterprise intelligence platform. Discover unsigned talent before your competition.
                    </p>

                    <button
                        onClick={onJoinWaitlist}
                        className="w-full py-4 bg-[#FF4500] text-black font-black uppercase tracking-widest rounded-xl hover:bg-[#FF4500]/90 transition-all"
                    >
...
// ============================================================================
// ARTIST CLAIM FORM - For artists to claim their profiles
// ============================================================================
const ArtistClaimForm = ({ artistName, artistId, onClose }: { artistName: string; artistId: string; onClose: () => void }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        // Save claim locally
        const claims = JSON.parse(localStorage.getItem('ss_claims_v1') || '[]');
        claims.push({ artistId, artistName, email, timestamp: new Date().toISOString() });
            ...
            <div className="bg-[#12121a] border border-white/10 rounded-2xl max-w-lg w-full p-8 relative">
                ...
                {submitted ? (
                    <div className="text-center py-8">
                        <Sparkles className="w-12 h-12 text-[#FF4500] mx-auto mb-4" />
                        <h2 className="text-2xl font-black text-white mb-2">Claim Submitted!</h2>
                        <p className="text-slate-400">We'll verify your profile and get back to you soon.</p>
                    </div>
                ) : (
                    <>
                        <h2 className="text-2xl font-black text-white uppercase tracking-tight mb-2">Claim Your Profile</h2>
                        <p className="text-slate-400 mb-6">Claiming <span className="text-[#FF4500] font-bold">{artistName}</span></p>
                        ...
                        <form onSubmit={handleSubmit}>
                            <input
                                ...
                                required
                                className="w-full px-4 py-3 bg-black/50 border border-white/10 rounded-xl text-white placeholder-slate-500 focus:border-[#FF4500]/50 focus:outline-none mb-4"
                            />
                            <button
                                type="submit"
                                className="w-full py-4 bg-[#FF4500] text-black font-black uppercase tracking-widest rounded-xl hover:bg-[#FF4500]/90 transition-all"
...
function JoinModal({ onClose }: { onClose: () => void }) {
    ...
    // Handle dismiss (user closes without submitting)
    const handleDismiss = () => {
        localStorage.setItem('ss_email_dismissed', 'true');
        onClose();
    };
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-2xl z-50 flex items-center justify-center p-4 animate-fade-in">
            <div className="glass max-w-md w-full p-8 rounded-3xl animate-slide-up shadow-2xl relative overflow-hidden">
                ...
                <div className="flex justify-between items-start mb-6">
                    ...
                    <button onClick={handleDismiss} className="p-2 text-slate-500 hover:text-white rounded-lg hover:bg-surface">
                        <X className="w-5 h-5" />
                    </button>
                </div>
...
function DossierModal({ artist, onClose, onClaim, isClaimed, onClaimProfile }: { artist: PowerIndexArtist, onClose: () => void, onClaim: (id: string) => void, isClaimed: boolean, onClaimProfile?: () => void }) {
    return (
                        ...
                        <div className="flex-1 text-center md:text-left">
                            ...
                            {onClaimProfile && (
                                <button
                                    ...
                                    className="mt-6 px-6 py-3 bg-amber-500/10 hover:bg-amber-500 text-amber-500 hover:text-black font-bold rounded-xl text-xs uppercase tracking-widest border border-amber-500/30 transition-all group"
                                >
...
const StelarEmblem = ({ className = "w-7 h-7" }: { className?: string }) => (
    <svg
        viewBox="0 0 100 100"
        className={className}
        fill="none"
    ...
    >
        <path d="M50 5L15 25V75L50 95L85 75V25L50 5Z" fill="#1C1C1E" />
        <path d="M50 5L15 25V75L50 95L85 75V25L50 5Z" stroke="#FF4500" strokeWidth="2" />
        <path d="M30 40C30 40 40 30 50 40C60 50 70 40 70 40" stroke="#FF4500" strokeWidth="4" strokeLinecap="round" />
        <path d="M30 50C30 50 40 40 50 50C60 60 70 50 70 50" stroke="#FF4500" strokeWidth="4" strokeLinecap="round" opacity="0.6" />
        <path d="M30 60C30 60 40 50 50 60C60 70 70 60 70 60" stroke="#FF4500" strokeWidth="4" strokeLinecap="round" opacity="0.3" />
    </svg>
);
...
export default function App() {
    ...
    const [showARLanding, setShowARLanding] = useState(false);
    const [showArtistClaim, setShowArtistClaim] = useState(false);
    ...
    // HOT 500: Top songs from iTunes, re-ranked by STELAR velocity
    const [hotSongs, setHotSongs] = useState<any[]>([]);
    const [hotSongsLoading, setHotSongsLoading] = useState(false);
    ...
    useEffect(() => {
        async function loadArtists() {
            ...
            if (activeTab === 'the-radar') {
                // The Radar uses the 'radar' key in rankings.json
                // Filter for Western artists only (US, Canada, UK, Europe)
                const westernCountries = [
                    'USA', 'US', 'United States', 'Canada', 'UK', 'United Kingdom', 'England',
                    'Germany', 'France', 'Spain', 'Italy', 'Netherlands', 'Belgium', 'Sweden',
                    'Norway', 'Denmark', 'Ireland', 'Scotland', 'Wales', 'Austria', 'Switzerland',
                    'Portugal', 'Poland', 'Finland', 'Australia', 'New Zealand'
                ];
                const radarArtists = rankingsData.rankings.radar || [];
                data = radarArtists.filter((artist: PowerIndexArtist) => {
                    const country = artist.country?.toUpperCase() || '';
                    return westernCountries.some(wc => country.includes(wc.toUpperCase()));
                });
            } else if (activeTab === 'sonic-signals') {
                data = rankingsData.rankings.arbitrage || [];
            } else {
                // THE PULSE uses global pool
    ...
    // HOT 500: Fetch top songs from iTunes RSS
    useEffect(() => {
        if (activeTab === 'hot-500' && hotSongs.length === 0) {
            setHotSongsLoading(true);

            // Fetch multiple iTunes Top Songs feeds for comprehensive coverage
            Promise.all([
                fetch('https://itunes.apple.com/us/rss/topsongs/limit=200/json').then(r => r.json()).catch(() => null),
                fetch('https://itunes.apple.com/us/rss/topsongs/limit=100/genre=18/json').then(r => r.json()).catch(() => null), // Hip-Hop
                fetch('https://itunes.apple.com/us/rss/topsongs/limit=100/genre=15/json').then(r => r.json()).catch(() => null), // R&B
                fetch('https://itunes.apple.com/us/rss/topsongs/limit=100/genre=6/json').then(r => r.json()).catch(() => null),  // Country
                fetch('https://itunes.apple.com/us/rss/topsongs/limit=100/genre=14/json').then(r => r.json()).catch(() => null), // Pop
            ])
                .then((results) => {
                    const parseSongs = (data: any) => {
                        return data?.feed?.entry?.map((item: any, index: number) => ({
                            id: item.id?.attributes?.['im:id'] || `song-${index}-${Date.now()}`,
                            name: item['im:name']?.label || 'Unknown',
                            artist: item['im:artist']?.label || 'Unknown Artist',
                            artistId: item['im:artist']?.attributes?.href?.match(/id(\d+)/)?.[1] || null,
                            artwork: item['im:image']?.[2]?.label?.replace('170x170', '600x600') || '',
                            genre: item.category?.attributes?.label || 'Music',
                            link: item.link?.attributes?.href || '#',
                            previewUrl: item.link?.find?.((l: any) => l.attributes?.type?.includes('audio'))?.attributes?.href || null,
                            rank: index + 1
                        })) || [];
                    };

                    // Combine all songs
                    const allSongs = results.flatMap(data => parseSongs(data));
                    ...
                    // STELAR Velocity Re-ranking: Shuffle slightly based on genre diversity
                    // Songs from genre-specific feeds get a small boost to surface variety
                    const rankedSongs = deduped.slice(0, 500).map((song, idx) => ({
                        ...song,
                        rank: idx + 1
                    }));

                    console.log(`[STELAR] HOT 500 loaded: ${rankedSongs.length} songs`);
                    setHotSongs(rankedSongs);
                    setHotSongsLoading(false);
                })
                .catch(err => {
                    console.error('Failed to load HOT 500:', err);
                    setHotSongsLoading(false);
                });
        }
    }, [activeTab, hotSongs.length]);
    ...
    const filteredArtists = useMemo(() => {
        ...
        if (activeTab === 'the-radar') {
            ...
            result = [...result].sort((a, b) => {
                const aPotential = a.is_independent ? (a.growthVelocity || 0) * 2 : (a.growthVelocity || 0);
                const bPotential = b.is_independent ? (b.growthVelocity || 0) * 2 : (b.growthVelocity || 0);
                return bPotential - aPotential;
            });
        }

        // STELAR PROPRIETARY: HOT 500 Algorithm
        // Sort by Growth Velocity to find hottest signals
        if (activeTab === 'hot-500') {
            result = [...result].sort((a, b) => (b.growthVelocity || 0) - (a.growthVelocity || 0));
        }
        ...
        setDisplayLimit(activeTab === 'the-pulse' ? 3000 : (activeTab === 'the-radar' ? 150 : (activeTab === 'hot-500' ? 500 : 50)));
        return result;
    ...
    // AUTO-TRIGGER EMAIL CAPTURE: Show modal after 20 seconds for new visitors
    // who haven't already joined the waitlist
    useEffect(() => {
        // Skip if user already captured or if not yet entered the main app
        const alreadyCaptured = localStorage.getItem('ss_waitlist') || localStorage.getItem('ss_email_dismissed');
        if (alreadyCaptured || !hasEntered) return;

        const timer = setTimeout(() => {
            // Double-check before showing
            const stillNotCaptured = !localStorage.getItem('ss_waitlist') && !localStorage.getItem('ss_email_dismissed');
            if (stillNotCaptured && !showJoin) {
                setShowJoin(true);
            }
        }, 20000); // 20 seconds
    ...
    return (
        <>
            ...
            {showARLanding && (
                <ARLandingPage
                    onClose={() => setShowARLanding(false)}
                    onJoinWaitlist={() => {
                        setShowARLanding(false);
                        setShowJoin(true);
                    }}
                />
            )}
            ...
            {/* Artist Claim Form */}
            {showArtistClaim && selectedArtist && (
                <ArtistClaimForm
                    artistName={selectedArtist.name}
                    artistId={selectedArtist.id}
                    onClose={() => setShowArtistClaim(false)}
                />
            )}
                ...
                <div className="flex h-screen overflow-hidden relative z-10"> {/* Subtract ticker height approx */}
                    ...
                    `}>
                                ...
                                <nav className="space-y-1">
                                    ...
                                    >
                                        ...
                                        {activeTab === 'the-radar' ? (
                                            ...
                                            <span className="text-[8px] text-accent font-black tracking-widest animate-pulse">PROPRIETARY</span>
                                        )}
                                    </button>
                        ...
                        <div className="mt-auto px-4 py-4 border-t border-white/5 space-y-3">
                            ...
                            <button
                                onClick={() => setShowARLanding(true)}
                                className="w-full p-3 rounded-xl bg-white/5 border border-white/10 hover:border-cyan-500/30 transition-all group"
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="text-left flex-1 min-w-0">
                                        <div className="text-xs font-black text-white uppercase">For Labels</div>
                                        <div className="text-[9px] text-cyan-400 font-bold">A&R Intelligence</div>
                                    </div>
                                                        ...
                                                        >
                                                            ...
                                                            <div className="mt-6 flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity">
                                                                <div className="text-[8px] font-black text-slate-700 uppercase tracking-widest flex items-center gap-2">
                                                                    <Cpu className="w-3 h-3" /> System: {(artist.status || 'Emerging').toUpperCase()}
                                                                </div>
                                                ...
                                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                                                    <div>
                                                        <h1 className="text-4xl font-black text-white mb-2 uppercase tracking-tighter flex items-center gap-3">
                                                            ...
                                                            <div className="flex items-center gap-1.5 ml-2">
                                                                <div className="w-2 h-2 rounded-full bg-[#FF4500] animate-pulse shadow-[0_0_10px_rgba(255,95,21,0.5)]"></div>
                                                                <span className="text-[10px] text-[#FF4500] font-mono uppercase tracking-widest">Top Songs</span>
                                                            </div>
                                                        </h1>
                                                        <p className="text-slate-500 text-sm font-medium">
                                                            The hottest songs on the planet right now, powered by STELAR Intelligence.
                                                        </p>
                                                    </div>
                                                    <div className="text-xs text-[#FF4500] font-mono font-black">{hotSongs.length} SONGS</div>
                                                </div>

                                                {hotSongsLoading ? (
                                                    <div className="text-center py-20">
                                                        <div className="inline-block w-8 h-8 border-2 border-accent/20 border-t-accent rounded-full animate-spin mb-4" />
                                                        <div className="text-slate-500 uppercase tracking-widest text-[10px] font-black">Loading HOT 500...</div>
                                                    </div>
                                                ) : (
                                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-16 py-10 border-t border-white/5">
                                                        {hotSongs.map((song, i) => (
                                                            ...
                                                            >
                                                                <div className="flex items-start gap-6 mb-6">
                                                                    <div className="relative w-24 h-24 shrink-0">
                                                                        <img
                                                                            ...
                                                                            alt={song.name}
                                                                            className="w-full h-full rounded-2xl object-cover grayscale brightness-75 group-hover:grayscale-0 group-hover:brightness-100 transition-all duration-700 group-hover:scale-105 group-hover:shadow-2xl"
                                                                            onError={(e) => { e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(song.name)}&background=1a1a2e&color=FF4500&size=96`; }}
                                                                        />
                                                                        <div className="absolute -top-2 -left-2 w-8 h-8 bg-[#FF4500] text-white rounded-full flex items-center justify-center text-[10px] font-black shadow-xl">
                                                                            {i + 1}
                                                                        </div>
                                                                    </div>
                                                                    <div className="flex-1 min-w-0">
                                                                        <div className="text-[10px] text-[#FF4500] font-black uppercase tracking-[0.2em] mb-2">{song.genre}</div>
                                                                        <h3 className="text-xl font-black text-white uppercase tracking-tighter truncate group-hover:text-[#FF4500] transition-colors leading-tight">{song.name}</h3>
                                                                        <div className="text-sm text-slate-400 font-medium mt-1 truncate">{song.artist}</div>
                                                                    </div>
                                                                </div>

                                                                <div className="flex items-center gap-3 mb-4">
                                                                    <a
                                                                        href={`/track/${song.artist.toLowerCase().replace(/\s+/g, '-')}/${song.name.toLowerCase().replace(/\s+/g, '-')}`}
                                                                        className="flex-1 px-4 py-3 bg-accent hover:bg-white text-black rounded-full text-[9px] font-black uppercase tracking-widest text-center transition-all flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(255,69,0,0.3)] hover:shadow-[0_0_20px_rgba(255,255,255,0.4)]"
                                                                    >
                                                                    ...
                                                                    <a
                                                                        ...
                                                                        rel="noopener noreferrer"
                                                                        onClick={(e) => e.stopPropagation()}
                                                                        className="flex-1 px-4 py-3 bg-white/5 hover:bg-white/10 text-white rounded-full text-[9px] font-black uppercase tracking-widest text-center transition-all border border-white/10 flex items-center justify-center gap-2"
                                                                    >
                                                                ...
                                                                <div className="text-accent text-[9px] font-black uppercase tracking-widest flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                                    View Artist <ChevronRight className="w-3 h-3" />
                                                                </div>
                                                            </div>
                                                        ))}
                                                    </div>
            ...
            {/* Analytics removed for local dev */}
        </>
    );
}
...
const TopTracks = ({ artist }: { artist: PowerIndexArtist }) => {
    ...
    useEffect(() => {
        const fetchTracks = async () => {
            ...
            try {
                ...
                // Strategy 1: Try our proxy first, then fallback to direct iTunes via CORS proxy
                let data: any = { results: [] };

                // Try our own proxy first
                try {
                    const res = await fetch(`/api/itunes?term=${encodeURIComponent(artistName)}&entity=song&limit=100`);
                    if (res.ok) {
                        data = await res.json();
                    }
                } catch (e) {
                    console.log('Our proxy failed, trying direct');
                }

                // If our proxy failed, try direct iTunes (works for some browsers)
                if (!data.results || data.results.length === 0) {
                    try {
                        const directRes = await fetch(`https://itunes.apple.com/search?term=${encodeURIComponent(artistName)}&entity=song&limit=100`);
                        if (directRes.ok) {
                            data = await directRes.json();
                        }
                    } catch (e) {
                        console.log('Direct iTunes also failed');
                    }
                }
                ...
                if (foundTracks.length === 0) {
                    const broadRes = await fetch(`/api/itunes?term=${encodeURIComponent(artistName)}&limit=100`);
    ...
    const [showAll] = useState(true); // SHOW 100 BY DEFAULT AS REQUESTED
            ...
            <div className="max-w-7xl mx-auto px-6">
                ...
                <div className="py-24 grid grid-cols-2 md:grid-cols-12 gap-12 border-b border-white/5">
                    ...
                    <div className="col-span-2 md:col-span-4 pr-12">
                        <div className="flex items-center gap-3 mb-8">
                            <StelarEmblem className="w-8 h-8" />
