...
const FEEDS = {
    FBI: 'https://news.google.com/rss/search?q=site:fbi.gov&hl=en-US&gl=US&ceid=US:en', // Updated to Google News for fresh data
    NSA: 'https://news.google.com/rss/search?q=site:nsa.gov&hl=en-US&gl=US&ceid=US:en',
    DHS: 'https://news.google.com/rss/search?q=site:dhs.gov&hl=en-US&gl=US&ceid=US:en', // Updated to Google News for fresh data
    CISA: 'https://www.cisa.gov/cybersecurity-advisories/all.xml', // Verified - still works
    ...
    STATE: 'https://www.state.gov/rss-feed/department-press-briefings/feed/', // Verified
    ...
    USDT: 'https://news.google.com/rss/search?q=site:home.treasury.gov&hl=en-US&gl=US&ceid=US:en',
};
...
export async function GET(request: Request) {
    try {
        ...
        const filteredFailsafe = FAILSAFE_INTEL.filter(item =>
            agencyFilter === 'ALL' ||
            item.agency === agencyFilter ||
            (item.agency === 'White House' && agencyFilter === 'White House') ||
            (item.agency === 'State Dept' && agencyFilter === 'State Dept') ||
            (item.agency === 'Treasury' && agencyFilter === 'Treasury')
        );
...
function generateHistoricalItems(count: number, agencyFilter: string) {
    const items = [];
    const agencies = agencyFilter === 'ALL' ? ['CIA', 'FBI', 'NSA', 'DHS', 'ODNI', 'DOD'] : [agencyFilter];
