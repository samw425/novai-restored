...
import {
    US_INTEL_FEEDS,
    US_INTEL_TECH_KEYWORDS,
    US_INTEL_FAILSAFE,
    generateHistoricalItems
} from '@/lib/feeds/intel-data';
...
export async function GET(
    ...
    const agencyFilter = searchParams.get('agency') || 'ALL'; // used by us-intel

    // SPECIAL HANDLER: War Room
    if (category === 'war-room') {
        try {
    ...
    // SPECIAL HANDLER: US Intel
    if (category === 'us-intel') {
        return handleUSIntel(agencyFilter, page, limit);
    }

    // SPECIAL HANDLER: Robotics
    if (category === 'robotics') {
        try {
            const items = await fetchRoboticsFeed();
            return NextResponse.json({ items, meta: { total: items.length, timestamp: new Date().toISOString() } });
        } catch (error) {
            return NextResponse.json({ items: [] }, { status: 500 });
        }
    }

    // SPECIAL HANDLER: Videos
    if (category === 'videos') {
        return handleVideos();
    }

    // SPECIAL HANDLER: Global Demographics
    if (category === 'global-demographics') {
        return handleGlobalDemographics();
    }
...
async function handleUSIntel(agencyFilter: string, page: number, limit: number) {
    try {
        const feedResults: any[] = [];

        for (const [agency, url] of Object.entries(US_INTEL_FEEDS)) {
        ...
        const filteredFailsafe = US_INTEL_FAILSAFE.filter(item =>
        ...
        let processedItems = allFeedItems
            .map((item: any) => {
                ...
                let score = 0;
                US_INTEL_TECH_KEYWORDS.forEach(keyword => {
                    if (text.includes(keyword)) score += 10;
                });

                if (!item.novai_analysis) {
                    if (score > 15) item.novai_analysis = "CRITICAL TECH SIGNAL";
                    else if (score > 0) item.novai_analysis = "RELEVANT";
                }
                return { ...item, score };
            })
            .filter((item: any) => {
                ...
                return isValidDate && (item.score > 0 || ['CISA', 'FBI', 'NSA'].includes(item.agency));
            })
            .sort((a: any, b: any) => new Date(b.pubDate).getTime() - new Date(a.pubDate).getTime());

        const startIndex = (page - 1) * limit;
        const paginatedItems = processedItems.slice(startIndex, startIndex + limit);

        if (paginatedItems.length < limit && page > 1) {
            paginatedItems.push(...generateHistoricalItems(limit - paginatedItems.length, agencyFilter));
        }
        ...
        return NextResponse.json({ items: US_INTEL_FAILSAFE });
    }
}
...
async function handleGlobalDemographics() {
    const COUNTRIES = [
    ...
    const countriesWithFlux = COUNTRIES.map((c: any) => ({
        ...c,
        population: c.population + Math.floor(Math.random() * 10) - 3
    }));

    const totalPopulation = 8118000000 + (Math.floor(Date.now() / 1000) % 100000) + Math.floor(Math.random() * 500);
    ...
    return NextResponse.json({
        ...
        globalStats: {
            totalPopulation,
            avgGrowth,
            avgUrban: Math.round(countriesWithFlux.reduce((acc, curr) => acc + (curr.urban || 0), 0) / countriesWithFlux.length),
            avgDensity: 145.2,
            avgMedianAge: (countriesWithFlux.reduce((acc, curr) => acc + (curr.medianAge || 0), 0) / countriesWithFlux.length).toFixed(1),
